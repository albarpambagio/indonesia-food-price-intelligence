...
def setup():
    ...
    DASH_MAP = dict(
        zip(["Rice", "Cooking Oil", "Sugar", "Flour"], ["solid", "dash", "dot", "dashdot"])
    )
    SYMBOL_MAP = dict(
        zip(
...
def islamic_data(mo, duckdb, pd, PROJECT_DB_PATH):
    ...
    def _query_islamic():
        with duckdb.connect(PROJECT_DB_PATH) as _c:
            return _c.sql(
                "SELECT year, eid_month, t_minus_1, t_minus_2, t_minus_3, t_plus_1 FROM wfp_intermediate.int_islamic_calendar ORDER BY year"
            ).fetchdf()
    ...
    mo.stop(
        len(islamic_df) == 0,
        mo.md(
            "⚠ **Islamic calendar data not found** — run `dbt seed` and `dbt run` to populate `int_islamic_calendar`."
        ),
    )
    return (islamic_df,)
...
def stakeholder_goals(df, mo, run_id, duckdb, PROJECT_DB_PATH):
    ...
    _pipeline_note = f"**Pipeline run**: `{run_id}`" if run_id else "*No pipeline lineage found*"
...
def coverage_commodity(df, mo):
    coverage = (
        df.groupby("commodity_consolidated")
        .agg(
            rows=("price", "count"),
            years=("year", "nunique"),
...
def coverage_island(df, mo):
    _gp = (
        df.dropna(subset=["island_group"])
        .groupby("island_group")
        .agg(
            rows=("price", "count"),
...
def filtered_data(df, commodity_dd, island_dd, year_slider):
    ...
    fdf = fdf[(fdf["year"] >= year_slider.value[0]) & (fdf["year"] <= year_slider.value[1])]
    return (fdf,)
...
def trend_charts(DASH_MAP, PALETTE_MAP, SYMBOL_MAP, fdf, mo, px):
    _yearly = (
        fdf.groupby(["year", "commodity_consolidated"])
        .agg(
            price=("price", "mean"),
    ...
    for _c in _yearly["commodity_consolidated"].unique():
        _fig_trend.update_traces(
            line=dict(dash=DASH_MAP[_c]),
            marker=dict(symbol=SYMBOL_MAP[_c]),
            name=_c,
            legendgroup=_c,
            selector=dict(name=_c),
        )
    ...
    _base = _yearly[_yearly["year"] == _yearly["year"].min()][
        ["commodity_consolidated", "price"]
    ].rename(columns={"price": "base"})
    ...
    _cagr = (
        _yearly.groupby("commodity_consolidated")
        .apply(
            lambda _g: (
                (
                    (
                        _g[_g["year"] == _g["year"].max()]["price"].values[0]
                        / _g[_g["year"] == _g["year"].min()]["price"].values[0]
                    )
                    ** (1 / (_g["year"].max() - _g["year"].min()))
                    - 1
                )
                * 100
            )
    ...
    *Source: WFP Indonesia Food Prices (HDX) | 2007–2024 | price_flag = actual*
    ...
    mo.md(
        "**USD-denominated** (FX-adjusted view — confirms IDR trends are real, not inflation-driven):"
    )
...
def volatility(PALETTE_MAP, fdf, fmt_idr, fmt_pct, mo, px):
    vol = (
        fdf.groupby(["year", "commodity_consolidated"])["price"].agg(["mean", "std"]).reset_index()
    )
    ...
    _fig_bar.update_layout(
        yaxis_title="CV% (Std Dev / Mean × 100)",
        xaxis=dict(dtick=1),
    )
    ...
    _avg = (
        vol.groupby("commodity_consolidated")["cv"]
        .mean()
        .reset_index()
...
def island_index(DASH_MAP, PALETTE_MAP, SYMBOL_MAP, df, fmt_pct, mo, px):
    _java_avg = (
        df[df["island_group"] == "Java"]
        .groupby(["year", "commodity_consolidated"])["price"]
        .mean()
        .reset_index()
    )
    _java_avg.columns = ["year", "commodity_consolidated", "java_price"]
    _island_avg = (
        df.groupby(["year", "commodity_consolidated", "island_group"])["price"].mean().reset_index()
    )
    ...
    _fig_island = px.line(
        ...
        y="price_index",
        color="island_group",
        facet_col="commodity_consolidated",
        facet_col_wrap=2,
...
def seasonality(DASH_MAP, PALETTE_MAP, SYMBOL_MAP, fdf, go, make_subplots, mo, np, px, MONTH_NAMES):
    _monthly = (
        fdf.groupby(["month", "commodity_consolidated"])["price"].agg(["mean", "std"]).reset_index()
    )
    ...
    for _c in ["Rice", "Cooking Oil", "Sugar", "Flour"]:
        ...
        if len(_p) > 0 and len(_t) > 0:
            _season_lines.append(
                f"- **{_c}**: peak {MONTH_NAMES[int(_p['month'].values[0]) - 1]} (IDR {_p['mean'].values[0]:,.0f}), trough {MONTH_NAMES[int(_t['month'].values[0]) - 1]} (IDR {_t['mean'].values[0]:,.0f})"
            )
...
def correlation(PALETTE_MAP, df, mo, px, pd, np, duckdb, PROJECT_DB_PATH):
    _pivot = df.pivot_table(
        index=["year", "month"], columns="commodity_consolidated", values="price", aggfunc="mean"
    ).reset_index()
...
def market_coverage(mo, pd, duckdb, PROJECT_DB_PATH):
    ...
    _east_markets = (
        market_cov.loc[market_cov["island_group"] == "Eastern Indonesia", "markets"].values[0]
        if "Eastern Indonesia" in market_cov["island_group"].values
        else 0
    )
...
def cooking_oil_shock(
    ...
    def _query_oil():
        ...
        _avg["ym"] = pd.to_datetime(
            _avg["year"].astype(str) + "-" + _avg["month"].astype(str).str.zfill(2) + "-01"
        )
        return _avg
...
def rice_harvest(PALETTE_MAP, df, fmt_idr, fmt_pct, go, mo, np, MONTH_NAMES):
    ...
    _fig_rice.add_trace(
        go.Bar(
            x=_rice_monthly["month"],
            y=_rice_monthly["mean"],
            name="Rice",
            marker_color=PALETTE_MAP["Rice"],
        )
    )
    _fig_rice.add_trace(
        go.Scatter(
            x=_rice_monthly["month"],
            y=_rice_monthly["mean"] + _rice_monthly["std"],
            mode="lines",
            line=dict(width=0),
            showlegend=False,
        )
    ...
    _fig_rice.add_trace(
        go.Scatter(
            x=_rice_monthly["month"],
            y=(_rice_monthly["mean"] - _rice_monthly["std"]).clip(lower=0),
            mode="lines",
            line=dict(width=0),
            showlegend=False,
            fill="tonexty",
    ...
    _fig_rice.update_layout(
        title=f"N2: Rice Prices {_gap:.1f}% Lower During Harvest Season (Mar–May)"
    )
...
def sugar_ramadan(PALETTE_MAP, df, fmt_idr, fmt_pct, go, mo, pd, MONTH_NAMES, islamic_df):
    ...
    _fig_sugar.add_trace(
        go.Bar(
            x=_sugar_monthly["month"],
            y=_sugar_monthly["price"],
            name="Sugar",
            marker_color=PALETTE_MAP["Sugar"],
        )
    )
    ...
    _fig_sugar.add_vrect(
        x0=_ramadan_start - 0.5,
        x1=_ramadan_end + 0.5,
        fillcolor="orange",
        opacity=0.06,
        line_width=0,
        annotation_text="Ramadan window (data-driven)",
        annotation_position="top left",
    )
...
def eastern_premium(DASH_MAP, PALETTE_MAP, SYMBOL_MAP, df, fmt_pct, mo, px, EASTERN_CUTOFF):
    _east = (
        df[df["island_group"] == "Eastern Indonesia"]
        .groupby(["year", "commodity_consolidated"])["price"]
        .mean()
        .reset_index()
    )
    ...
    _avg_premium = (
        _gap.groupby("commodity_consolidated")["premium_pct"]
        .mean()
        .reset_index()
...
def forecast_quality(fmt_idr, fmt_pct, mo, np, pd, json, os):
    ...
    mo.stop(
        not os.path.exists(_fc_path),
        mo.md(
            f"⚠ **Forecast file not found**: `{_fc_path}` — run `uv run python forecast/run_forecast.py` first."
        ),
    )
    ...
    for _c in _forecasts["commodity"].unique():
        ...
        _merged = _a.merge(
            _f[["date", "commodity", "forecast_price"]], on=["date", "commodity"], how="inner"
        )
        if len(_merged) > 0:
            ...
            _mae_table.append(
                {
                    "Commodity": _c,
                    "Holdout MAE (IDR)": round(_mae, 1),
                    "N (overlap months)": len(_merged),
                }
            )
        else:
            _mae_table.append(
                {"Commodity": _c, "Holdout MAE (IDR)": "N/A (no overlap)", "N (overlap months)": 0}
            )
    ...
    *Source: `dashboard/public/data/forecast.json` — 6-month horizon, 12-month holdout*
    ...
    for _c, _v in _meta.get("models", {}).items():
        _models.append(
            {
                "Commodity": _c,
                "Selected Model": _v.get("selected", "?"),
                "Holdout MAE": round(_v.get("holdout_mae", 0), 1),
            }
        )
    if _models:
...
def q1_annual_trend(fmt_pct, fmt_cagr, fdf, PALETTE_MAP, DASH_MAP, SYMBOL_MAP, mo, px, pd):
    ...
    _fig.add_vline(
        x=2008, line_dash="dash", line_color="gray", opacity=0.4, annotation_text="2008 Food Crisis"
    )
    _fig.add_vline(
        x=2022,
        line_dash="dash",
        line_color="gray",
        opacity=0.4,
        annotation_text="2022 Cooking Oil Shock",
    )
    _fig.update_layout(
        yaxis_title="Avg Price (IDR)", yaxis=dict(tickformat="~s"), xaxis=dict(dtick=2)
    )
    ...
    for _c in _yearly["commodity_consolidated"].unique():
        ...
        _cagr_data.append(
            {"Commodity": _c, "CAGR": _cagr, "Start Price": _start, "End Price": _end}
        )
...
@app.cell
def q1_trend_decomposition(
    fmt_pct, fmt_idr, fdf, PALETTE_MAP, MONTH_NAMES, mo, go, make_subplots, pd, np
):
    _decomp_data = (
        fdf.groupby(["year", "month", "commodity_consolidated"])["price"].mean().reset_index()
    )
    _decomp_data["ym_num"] = (
        _decomp_data["year"] - _decomp_data["year"].min()
    ) * 12 + _decomp_data["month"]
    ...
    for _c in _decomp_data["commodity_consolidated"].unique():
        ...
        _d["residual"] = _d["detrended"] - _d["month"].map(
            _monthly_avg.set_index("month")["detrended"]
        )
        ...
        _dates_for_plot = pd.to_datetime(
            _d["year"].astype(str) + "-" + _d["month"].astype(str) + "-01"
        )
        ...
        _fig.add_trace(
            go.Scatter(
                ...
                mode="lines",
                name="Observed",
                line=dict(color=PALETTE_MAP[_c]),
            ),
            row=1,
            col=1,
        )
        ...
        _fig.add_trace(
            go.Scatter(
                ...
                mode="markers",
                name="Residual",
                marker=dict(color=PALETTE_MAP[_c], size=3, opacity=0.6),
            ),
            row=4,
            col=1,
        )
        ...
        _peak_m = MONTH_NAMES[
            int(_monthly_avg.loc[_monthly_avg["detrended"].idxmax(), "month"]) - 1
        ]
        _trough_m = MONTH_NAMES[
            int(_monthly_avg.loc[_monthly_avg["detrended"].idxmin(), "month"]) - 1
        ]
        _residual_std = _d["residual"].std()

        _figures[_c] = _fig
        _insights.append(
            f"- **{_c}**: seasonal range IDR {_seasonal_range:,.0f}, peak {_peak_m}, trough {_trough_m}, residual noise ±IDR {_residual_std:,.0f}"
        )
...
def q1_forecast_overlay(fmt_idr, fmt_pct, PALETTE_MAP, DASH_MAP, SYMBOL_MAP, mo, go, pd, json, os):
    ...
    mo.stop(
        not os.path.exists(fc_path),
        mo.md(
            f"⚠ **Forecast file not found**: `{fc_path}` — run `uv run python forecast/run_forecast.py` first."
        ),
    )
    ...
    for _c in _forecast_df["commodity"].unique():
        ...
        _fig.add_vline(
            x=pd.Timestamp(_f["date"].min()), line_dash="dash", line_color="gray", opacity=0.5
        )
        ...
        _last_actual = _a_recent[_a_recent["date"] == _a_recent["date"].max()][
            "actual_price"
        ].values
        _first_fc = _f[_f["date"] == _f["date"].min()]["forecast_price"].values
        _last_fc = _f[_f["date"] == _f["date"].max()]["forecast_price"].values
        _last_lower = (
            _f[_f["date"] == _f["date"].max()]["lower_95"].values
            if "lower_95" in _f.columns
            else [None]
        )
        _last_upper = (
            _f[_f["date"] == _f["date"].max()]["upper_95"].values
            if "upper_95" in _f.columns
            else [None]
        )
    ...
    mo.md(
        "**Procurement Action Assessment** (current price → forecast trajectory):"
        + "\n\n"
        + "\n\n".join(_zone_insights)
        + """
...
@app.cell
def q2_month_of_year(
    fmt_idr, fmt_pct, fdf, PALETTE_MAP, DASH_MAP, SYMBOL_MAP, MONTH_NAMES, mo, go, make_subplots
):
    _monthly = (
        fdf.groupby(["month", "commodity_consolidated"])["price"].agg(["mean", "std"]).reset_index()
    )
    ...
    for _pos, _c in enumerate(["Rice", "Cooking Oil", "Sugar", "Flour"]):
        ...
        _fig.update_xaxes(
            tickmode="array", tickvals=list(range(1, 13)), ticktext=MONTH_NAMES, row=_r, col=_col
        )
        _fig.update_yaxes(tickformat="~s", row=_r, col=_col)

    _fig.update_layout(
        title="Q2.1: Month-of-Year Seasonality (±1 Std Dev)", height=500, template="plotly_white"
    )
    ...
    for _c in ["Rice", "Cooking Oil", "Sugar", "Flour"]:
        ...
        if len(_p) > 0 and len(_t) > 0:
            ...
            _lines.append(
                f"- **{_c}**: peak {MONTH_NAMES[int(_p['month'].values[0]) - 1]} ({fmt_idr(_p['mean'].values[0])}), trough {MONTH_NAMES[int(_t['month'].values[0]) - 1]} ({fmt_idr(_t['mean'].values[0])}), gap {fmt_pct(_gap)}"
            )
...
@app.cell
def q2_ramadan_analysis(
    fmt_idr, fmt_pct, fdf, PALETTE_MAP, DASH_MAP, SYMBOL_MAP, MONTH_NAMES, mo, go, pd, islamic_df
):
    _df = fdf.copy()
    ...
    for _c_comm in ["Rice", "Cooking Oil", "Sugar", "Flour"]:
        ...
        for _col, _label in ramadan_window.items():
            ...
            if len(_match) > 0:
                ...
                _premium = (
                    (_merged["price"].mean() / _merged["price_annual"].mean() - 1) * 100
                    if len(_merged) > 0
                    else 0
                )
                _n = len(_match)
                _window_data.append(
                    {
                        "Commodity": _c_comm,
                        "Window": _label,
                        "Avg Price": round(_match["price"].mean(), 0),
                        "Premium vs Annual": round(_premium, 1),
                        "N": _n,
                    }
                )
...
def q2_harvest_year_end(fmt_pct, fmt_idr, fdf, PALETTE_MAP, MONTH_NAMES, mo, go):
    ...
    _annual_avg = (
        fdf.groupby(["year", "commodity_consolidated"])["price"]
        .mean()
        .reset_index()
        .rename(columns={"price": "annual_avg"})
    )
    _monthly = _monthly.merge(
        _annual_avg.groupby("commodity_consolidated")["annual_avg"].mean().reset_index(),
        on="commodity_consolidated",
        how="left",
    )
    ...
    _fig.add_vrect(
        ...
        opacity=0.06,
        line_width=0,
        annotation_text="Harvest (Mar–May)",
        annotation_position="top left",
    )
    _fig.add_vrect(
        x0=10.5,
    ...
    _fig.update_layout(
        ...
        xaxis=dict(tickmode="array", tickvals=list(range(1, 13)), ticktext=MONTH_NAMES),
        yaxis_title="Price Index (Annual Avg = 100)",
        template="plotly_white",
        height=400,
    )
    ...
    for _c in ["Rice", "Cooking Oil", "Sugar", "Flour"]:
        ...
        _harvest_discounts.append(
            f"- **{_c}**: harvest index {_h_avg:.1f} (discount {fmt_pct(100 - _h_avg)}), year-end index {_ye_avg:.1f}"
        )
...
@app.cell
def q3_island_group_index(
    fmt_pct, fdf, PALETTE_MAP, DASH_MAP, SYMBOL_MAP, mo, px, pd, np, scipy_stats
):
    _java_avg = (
        fdf[fdf["island_group"] == "Java"]
        .groupby(["year", "commodity_consolidated"])["price"]
        .mean()
        .reset_index()
    )
    _java_avg.columns = ["year", "commodity_consolidated", "java_price"]
    _island_avg = (
        fdf.groupby(["year", "commodity_consolidated", "island_group"])["price"]
        .mean()
        .reset_index()
    )
    _index = _island_avg.merge(_java_avg, on=["year", "commodity_consolidated"], how="left")
    ...
    for _c in ["Rice", "Cooking Oil", "Sugar", "Flour"]:
        ...
        for _ig in _sub["island_group"].unique():
            ...
            if len(_g) >= 3:
                _slope, _intercept, _r_val, _p_val, _std_err = scipy_stats.linregress(
                    _g["year"], _g["price_index"]
                )
                _direction = "narrowing" if _slope < 0 else "widening"
                _trend_insights.append(
                    {
                        "Commodity": _c,
                        "Island Group": _ig,
                        "Avg Index": round(_g["price_index"].mean(), 1),
                        "Trend": _direction,
                        "Slope (ppt/yr)": round(_slope, 2),
                        "R²": round(_r_val**2, 2),
                    }
                )
    ...
    **Java = 100.** Values above 100 indicate a premium over Java; values below 100 indicate a discount.
    ...
    if len(_trend_df) > 0:
        ...
        _ = mo.hstack(
            [
                mo.ui.table(
                    _trend_df.sort_values(["Commodity", "Island Group"]).round(1),
                    label="Island Group Premium Trends",
                ),
                mo.md(f"""
            > **Insight:**
...
def q3_province_drilldown(fmt_idr, fdf, mo, pd, np, EASTERN_CUTOFF):
    ...
    _prov_avg = (
        _filtered.groupby(["commodity_consolidated", "island_group", "admin1"])["price"]
        .agg(["mean", "count", "std"])
        .reset_index()
    )
    ...
    for _c in _prov_avg["Commodity"].unique():
        ...
        **{_c}** — Cheapest vs Most Expensive
        ...
        mo.hstack(
            [
                mo.ui.table(
                    _cheapest[["Province", "Island Group", "Avg Price", "Records"]].round(0),
                    label=f"{_c} — Top 5 Cheapest",
                ),
                mo.ui.table(
                    _dearest[["Province", "Island Group", "Avg Price", "Records"]].round(0),
                    label=f"{_c} — Top 3 Most Expensive",
                ),
            ],
            justify="start",
        )
...
def q4_lagged_correlation(fmt_pct, mo, px, pd, json, os):
    ...
    mo.stop(
        not os.path.exists(cs_path),
        mo.md(
            f"⚠ **Correlation summary not found**: `{cs_path}` — run `uv run python export/export_json.py` first."
        ),
    )
    ...
    _best = _df_cs.loc[_df_cs.groupby("commodity_pair")["pearson_r"].idxmax()].sort_values(
    ...
    mo.ui.plotly(_fig_lag)
    mo.ui.table(
        _best[["commodity_pair", "lag_months", "pearson_r"]].round(3).reset_index(drop=True),
        label="Best Lag per Pair",
    )
...
def q4_rolling_stability(fmt_pct, fdf, PALETTE_MAP, DASH_MAP, mo, go, pd, np):
    ...
    for _c1, _c2 in _pairs_list:
        ...
        for _i in _rolling.dropna().index:
            ...
            if _mid_idx in _d.index:
                ...
                _mid_dates.append(
                    f"{int(_d.loc[_mid_idx, 'year'])}-{int(_d.loc[_mid_idx, 'month']):02d}"
                )
        _rolling_results.append(
            {
                "pair": f"{_c1} ↔ {_c2}",
                "dates": _mid_dates,
                "corrs": _corrs,
                "mean_r": np.mean(_corrs),
                "std_r": np.std(_corrs),
            }
        )
    ...
    _fig.add_hline(
        y=0.5, line_dash="dot", line_color="gray", opacity=0.4, annotation_text="r = 0.5"
    )
    _fig.add_hline(
        y=-0.5, line_dash="dot", line_color="gray", opacity=0.4, annotation_text="r = -0.5"
    )
    ...
    _fig.update_layout(
        ...
        yaxis_title="Pearson r (36-month window)",
        template="plotly_white",
        height=450,
    )
    ...
    for _r in _rolling_results:
        ...
        _stab_lines.append(
            f"- **{_r['pair']}**: mean r = {_r['mean_r']:.3f} ± {_r['std_r']:.3f} (CV = {fmt_pct(_cv)})"
        )
    mo.md("\n".join(_stab_lines))

    _ym_2022_min = (
        _pivot.loc[_pivot["year"] == 2022, "ym_num"].min()
        if _pivot["year"].eq(2022).any()
        else None
    )
    _pre_post = []
    for _c1, _c2 in _pairs_list:
        if _ym_2022_min is None:
            ...
            _change = (
                round(_r_post - _r_pre, 3)
                if isinstance(_r_pre, float) and isinstance(_r_post, float)
                else "N/A"
            )
            _pre_post.append(
                {
...
def reconciliation(mo, pd, json, os, duckdb, PROJECT_DB_PATH):
    def _build():
        ...
        def _query_mart_rows():
            ...
            with duckdb.connect(PROJECT_DB_PATH) as _c:
                ...
                for _sch in _schemas:
                    _tables = _c.sql(
                        f"SELECT table_name FROM information_schema.tables WHERE table_schema = '{_sch}'"
                    ).fetchdf()
        ...
        try:
            _json_files = [
                f for f in os.listdir(_json_dir) if f.endswith(".json") and f != "forecast.json"
            ]
        ...
        _summary = (
            "All exports verified — mart row counts match JSON record counts."
            if _all_ok
            else "Mismatch detected — some JSON files don't match source marts. Re-run export."
        )

        _layers = [
            "raw.food_prices",
            "raw.markets",
            "wfp_staging.stg_food_prices",
            "wfp_staging.stg_markets",

...
def load_data(duckdb, mo, PROJECT_DB_PATH):
    ...
    commodity_dd = mo.ui.dropdown(commodities, value="Rice", label="Select commodity")
...
@app.cell
def controls(mo):
    holdout_months = mo.ui.number_slider(start=3, stop=24, step=1, value=12, label="Holdout months")
    forecast_horizon = mo.ui.number_slider(
        start=3, stop=12, step=1, value=6, label="Forecast horizon (months)"
    )
    season_length = mo.ui.number_slider(start=4, stop=24, step=1, value=12, label="Season length")
    use_islamic = mo.ui.checkbox(value=True, label="Islamic calendar flags as exogenous regressors")
    include_theta = mo.ui.checkbox(value=False, label="Include AutoTheta (slower)")
    return (
        forecast_horizon,
        holdout_months,
        include_theta,
...
def display_controls(
    data_info, forecast_horizon, holdout_months, include_theta, mo, season_length, use_islamic
):
    mo.vstack(
        [
            data_info,
...
def holdout_evaluation(
    ...
    models = [
        AutoARIMA(season_length=int(season_length.value)),
...
def final_forecast(
    ...
    if exog_cols:
        ...
        for _flag in ["eid", "t1", "t2", "t3"]:
            ...
            for _, _row in cal.iterrows():
                fdf.loc[
                    fdf["ym"]
                    == _row[
                        _flag.replace("eid", "eid_month")
                        .replace("t1", "t_minus_1")
                        .replace("t2", "t_minus_2")
                        .replace("t3", "t_minus_3")
                    ],
                    _flag,
                ] = 1
        fcast = sf_final.forecast(
            ...
            X_df=fdf[["unique_id", "ds", *exog_cols]],
            level=[95],
        )
    else:
...
def plot_forecast(model_cols, fcast, go, selected, train, train_df):
    ...
    fig.add_trace(
        go.Scatter(
            x=fcast["ds"],
            y=fcast[model_cols[0]],
            mode="lines+markers",
            name=f"Forecast ({model_cols[0]})",
    ...
    if lo_col in fcast.columns:
        fig.add_trace(
            go.Scatter(
                ...
                mode="lines",
                name="Lower 95% CI",
                line=dict(color="#C44E52", width=1, dash="dash"),
            )
        )
        fig.add_trace(
            go.Scatter(
...
def cross_validation_analysis(
    ...
    cv_mae = (
        ...
        .apply(
            lambda g: {
                m: float(np.mean(np.abs(g["y"] - g[m])))
                for m in [
                    m
                    for m in cv_df.columns
                    if m not in ("unique_id", "ds", "y", "cutoff", "lo-95", "hi-95")
                ]
            }
        )
        .iloc[0]
